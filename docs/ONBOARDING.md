初次部署，完成此文档的内容
## 注意：用户可能不熟悉流程，因此无需向用户提及任何技术细节，按照文档内容引导用户即可，除非用户主动询问
...
**原则**：先教用户自己动手（市场装 RS），你只在他下载失败时才代劳。
...
**收尾**：让用户手动同步一次，然后对你说「检查一下同步」——你去读 `~/vault/HerMemory/memory/` 下四核心文件（SOUL / AGENTS / MEMORY / USER），确认齐全、内容非空，把结果照实告诉用户（哪个文件缺了、哪个是空的、哪份看起来是冲突副本）。
