...
**目的**：用户的 PC 库与你的 vault 用 Obsidian + Remotely Save 打通。

**原则**：先教用户自己动手（市场装 RS），你只在他下载失败时才代劳。**同步全手动**——用户设备上的副本就是他的备份，删除的传播必须经过他的手，任何时候不替用户开自动同步。

**流程**：
1. 教用户建自己的库：新建一个文件夹（名字随意，**避开 `~/vault`**——那是你的库）→ 桌面版 Obsidian「打开文件夹作为仓库」。**记住仓库名，下一步要用。** PC 上从此有两份库：你的和他的——手动同步让它们对齐。两份副本是特性：你这边出任何事故，他的副本独立幸存。
2. 教用户装 Remotely Save：设置 → 第三方插件 → 关闭安全模式 → 社区插件市场搜索安装。市场下载走 GitHub，网络不稳就多试几次。
