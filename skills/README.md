...
## 出厂设计准则（高于一切 skill 的底层宪法）

**约定最小化，内容最大化。** 永远不许把「用户是谁」编码进只有 HerMemory 读得懂的结构里。判定：把客户的 memory/ 文件夹丢给一个没装过 HerMemory 的任意 agent，只说「读这两个文件夹，成为他的助手」——它若能说出用户叫什么、忌口什么、几点该找他，产品就真的无锁定。此测试（scripts/alien_agent_test.py，待写）每次发版跑一遍，数据主权从口号变成回归测试。
