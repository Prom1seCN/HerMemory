...
#   落法：~/.hermes/AGENTS.md 软链 → 同步范围 memory/AGENTS.md（零代码走官方通道，行为规则槽位（原生命名））
# TODO(实测)：记忆软链五件——SOUL.md（身份槽根目录）、memories/MEMORY.md、memories/USER.md、AGENTS.md（=AGENTS 行为规则）、其余按需。
